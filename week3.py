...
# Base current for healthy nodes (stable fixed point)
I_base = 0.35
# Higher current for focus node (limit cycle/oscillatory regime)
I_focus = 1.0  
...
def fhn_dynamics(state, t, adj, coupling, currents):
    ...
    # Optimized matrix calculation for speed
...
current_state = np.random.rand(2, num_nodes) * 0.1
simulation_history = np.zeros((steps, num_nodes))
time_points = np.linspace(0, T, steps)

print(f"Simulating network with {num_nodes} nodes for {T}ms...")
...
# Identify which nodes crossed the seizure threshold
# We check if the max amplitude in the second half of sim > threshold
...
# Plot Distant Node (if available)
...
# --- Visualization: Spatiotemporal Heatmap ---

plt.figure(figsize=(14, 8))
sns.heatmap(simulation_history.T, cmap="rocket", center=0, cbar_kws={'label': 'Activity Amplitude (v)'})

plt.title('Spatiotemporal Spread of Seizure Activity')
plt.xlabel('Time Steps')
plt.ylabel('Node Index')

# Mark the focus node on the y-axis
plt.yticks([focus_node_index + 0.5], labels=[f'Focus {focus_node_index}'], rotation=0, color='red', fontweight='bold')
